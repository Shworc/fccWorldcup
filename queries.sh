...
echo "$($PSQL "SELECT SUM(winner_goals) FROM games WHERE winner_goals > 2")"
...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE games.year=2014 AND round='Eighth-Final';")"
